<?php
$rootUrl = url('/knowledge-styles/');
?>

<h2 style="font-size: 25px; margin-bottom: 10px;">Layout Examples</h2>

<ul class="styleGuideNav-list">
<!--    <li class="styleGuideNav-item">-->
<!--        <a class="styleGuideNav-link" href="--><?php //echo $rootUrl; ?><!--">Style Guide Home</a>-->
<!--    </li>-->
<!--    <li class="styleGuideNav-item">-->
<!--        <a class="styleGuideNav-link" href="--><?php //echo $rootUrl; ?><!--full">Full Width</a>-->
<!--    </li>-->
<!--    <li class="styleGuideNav-item">-->
<!--        <a class="styleGuideNav-link" href="--><?php //echo $rootUrl; ?><!--columnsTwo">Two Columns</a>-->
<!--    </li>-->
<!--    <li class="styleGuideNav-item">-->
<!--        <a class="styleGuideNav-link" href="--><?php //echo $rootUrl; ?><!--columnsThree">Three Columns</a>-->
<!--    </li>-->
<!--    <li class="styleGuideNav-item">-->
<!--        <a class="styleGuideNav-link" href="--><?php //echo $rootUrl; ?><!--panelLeft">Left Panel</a>-->
<!--    </li>-->
<!--    <li class="styleGuideNav-item">-->
<!--        <a class="styleGuideNav-link" href="--><?php //echo $rootUrl; ?><!--panelRight">Right Panel</a>-->
<!--    </li>-->
<!--    <li class="styleGuideNav-item">-->
<!--        <a class="styleGuideNav-link" href="--><?php //echo $rootUrl; ?><!--panelAndNav">Panel And Nav</a>-->
<!--    </li>-->
<!--</ul>-->


<h2 style="font-size: 25px; margin: 30px 0 10px;">Page Examples</h2>
<ul class="styleGuideNav-list">
    <li class="styleGuideNav-item">
        <a class="styleGuideNav-link" href="<?php echo $rootUrl;?>pageHome">Home</a>
    </li>
    <li class="styleGuideNav-item">
        <a class="styleGuideNav-link" href="<?php echo $rootUrl;?>pageArticle">Article</a>
    </li>
    <li class="styleGuideNav-item">
        <a class="styleGuideNav-link" href="<?php echo $rootUrl;?>pageCategory">Article</a>
    </li>
    <li class="styleGuideNav-item">
        <a class="styleGuideNav-link" href="<?php echo $rootUrl;?>pageCategoryWithNav">Article</a>
    </li>
</ul>
