...
//      $Expires = GetValue('expires', $Tokens, NULL);
   ...
   public function SignInButton($type = 'button') {
      ...
      $Title = sprintf(T('Sign in with %s'), 'LinkedIn');
      
      switch ($type) {
         case 'icon':
            $Src = $this->GetWebResource('design/linkedin-icon.png');
            break;
         case 'button':
         default:
            $Src = $this->GetWebResource('design/linkedin-button.png');
            break;
      }
      
      $Result = Anchor("<img src='$Src' />", $Url, '', array('title' => $Title));
      return $Result;
   }
