...
export function getError(errorMessageProps: IErrorMessageProps) {
    const errorCode = errorMessageProps.apiError
        ? errorMessageProps.apiError.response.status
        : errorMessageProps.defaultError;
    return {
        ...parseErrorCode(errorCode, errorMessageProps),
        ...(errorMessageProps.error || {}),
    };
}

function renderSignin(currentUser: ILoadable<IMe>) {
...
function parseErrorCode(errorCode?: string | number): IError {
