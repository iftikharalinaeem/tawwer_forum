        ...
        <?php foreach ($this->data('Statuses') as $row): ?>
            <tr id="Status_<?php echo $row['StatusID']; ?>">
                ...
                <td class="options">
                    ...
                    echo anchor(dashboardSymbol('delete'), '/settings/deletestatus?statusid='.$row['StatusID'], 'js-modal-confirm js-hijack btn btn-icon', ['aria-label' => t('Delete'), 'title' => t('Delete'), 'data-body' => sprintf(t('Are you sure you want to delete this %s?'), t('Status'))]);
                    ?>
                    </div>
                </td>
            </tr>
