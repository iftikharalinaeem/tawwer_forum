...
$Configuration = array();
//include_once(PATH_ROOT.DS.'conf'.DS.'config-defaults.php');
include_once(PATH_ROOT.DS.'conf'.DS.'config.php');
$DbConfig = $Configuration['Database'];
$Ex->PDO(sprintf('mysql:host=%s;dbname=%s;', str_replace(':', ';port=', $DbConfig['Host']), $DbConfig['Name']),
	$DbConfig['User'],
	$DbConfig['Password']);
$Ex->Prefix = array_key_exists('DatabasePrefix', $DbConfig) ? $DbConfig['DatabasePrefix'] : 'GDN_';
unset($Configuration);
unset($DbConfig);
...
echo '<pre>';
$Ex->BeginExport(dirname(__FILE__).DS.'uploads'.DS.'export '.date('Y-m-d His').'.txt.gz', 'Vanilla 2.0');

$Ex->ExportTable('User', 'select UserID, Email from :_User', array('Name' => 'Email')); // ":_" will be replace by database prefix
...
class vnExport {
	...
	public function ExportTable($TableName, $Query, $Mappings = array()) {
		...
		fwrite($fp, $TableName.self::NEWLINE);
		...
		$EscapeSearch = array(self::DELIM, self::ESCAPE, self::NEWLINE, self::QUOTE);
		$EscapeReplace = array(self::ESCAPE.self::DELIM, self::ESCAPE.self::ESCAPE, self::ESCAPE.self::NEWLINE, self::ESCAPE.self::QUOTE);
		
		// Write the structure as a comment for reference.
		$this->Comment(implode(self::DELIM, $Structure), FALSE);
	...
	protected $_Structures = array(
		...
		'Discussion' => array('DiscussionID', 'Name', 'CategoryID', 'DateInserted', 'InsertUserID', 'DateUpdated', 'UpdateUserID', 'Closed', 'Announce', 'Sink'),
