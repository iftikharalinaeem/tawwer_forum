        ...
        $File = '/srv/www/subdomains/' . $Item . '/conf/config.php';
		  if (file_exists($File)) {
				...
				$Contents = str_replace(
					 ...
					 '$Configuration['."'".'Database'."'".']['."'".'Host'."'".'] = '."'".'vfdb1'."'".';',
					 $Contents
				);
...
if ($DirectoryHandle = opendir('/srv/www/subdomains')) {
    while (($Item = readdir($DirectoryHandle)) !== FALSE) {
        $File = '/srv/www/subdomains/' . $Item . '/conf/config.php';
		  // if ($Item == 'marky' && file_exists($File)) {
		  if (file_exists($File)) {
				// Symlink the plugin
				exec('ln -s /srv/www/vanilla_source/plugins/downtime /srv/www/subdomains/'.$Item.'/plugins/downtime');
				// Enable the plugin
				$Contents = file_get_contents($File);
				$Contents = str_replace("// EnabledPlugins", '// EnabledPlugins
$Configuration['."'".'EnabledPlugins'."'".']['."'".'downtime'."'".'] = '."'".'downtime'."'".';', $Contents);
				
				// Remove the module sort order
				$Contents = str_replace('// Modules
...
$Configuration['."'".'Modules'."'".']['."'".'Conversations'."'".']['."'".'Content'."'".'] = array('."'".'Content'."'".', '."'".'Ads'."'".');', '', $Contents);
