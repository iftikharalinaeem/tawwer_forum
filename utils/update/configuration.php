...
class Configuration {
   ...
   public function Get($Name, $DefaultValue = FALSE) {
      ...
      if(is_string($Value))
         $Result = TaskList::Unserialize($Value);
      else
         $Result = $Value;
      ...
      for ($i = 0; $i < $KeyCount; ++$i) {
         ...
         if($i == $KeyCount - 1) {   
            ...
            if($KeyExists === FALSE || $Overwrite === TRUE) {
               $Array[$Key] = TaskList::Serialize($Value);
               $SaveArray[$Key] = TaskList::Serialize($Value);
            }
         } else {
   ...
   public function Save($File = '', $Group = '', $RequireSourceFile = TRUE) {
      ...
      if ($File == '')
         throw new Exception('You must specify a file path for your config file to be saved.');
      ...
      if ($FileContents === FALSE)
         throw new Exception('Failed to define configuration file contents.');
   ...
   private function _FormatArrayAssignment(&$Array, $Prefix, $Value) {
      if (is_array($Value)) {
         ...
         if (strpos($Value, "'") !== FALSE) {
            $Array[] = $Prefix .= ' = "'.TaskList::ArrayValueForPhp(str_replace('"', '\"', $Value)).'";';
         } else {
            $Array[] = $Prefix .= " = '".TaskList::ArrayValueForPhp($Value)."';";
         }
      }
   }
}
